...
If the inserted picture occupies a separate line and a title is set (`![optional picture description text] (picture file path "title set here")`), the picture will be automatically centered and the set title will be in The bottom of the picture is rendered as a "explanatory text", and the title part supports span-level typesetting, such as bold, italic, formula, etc.
